...
class Arah extends RestfullApi
{
	...
	public function create() {
        $nama = $this->request->getPost('nama_arah_jalan');
        if (empty($nama)) {
            return $this->respond(['status'=>false, 'message'=>'Ruas jalan masih kosong']);
        }
        if ($this->model->insert(['nama_arah_jalan'=>$nama])) {
    ...
    public function update($id = null) {
        $nama = $this->request->getVar('nama_arah_jalan');
        if (empty($id)) {
        ...
        if ($this->model->update($id, ['nama_arah_jalan'=>$nama])) {
