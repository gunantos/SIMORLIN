<template>
    <v-navigation-drawer app floating v-model="sidebar">
      <v-list-item>
        <v-list-item-content>
          <v-list-item-title class="text-h4" style="text-align: center;font-weight: 600;">
            {{ appname }}
          </v-list-item-title>
      ...
      <v-list
        ...
        nav
      >
      <div v-for="(item, inde) in items" 
          :key="inde">
        <v-list-group v-if="issub(item)" no-action sub-group>
           ...
           <v-list-item v-for="(itm, ind) in item.sub" :key="ind" link :to="itm.path">
